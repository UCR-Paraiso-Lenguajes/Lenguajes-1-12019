...
var PDFViewer = function PDFViewer(element) {
  ...
  var $navbar = this.$navbar = $('<div class="navbar navbar-inverse navbar-static-top"/>').appendTo($element);
  ...
  $('<li class="dropdown">' +
    '<a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-search-plus"/> <b class="caret"/></a>' +
    '<ul class="dropdown-menu pull-right">' +
      '<li><a href="#zoom" data-value="' + PDFViewer.Scale.AUTO + '"><i class="fa fa-check"/> Automatic</a></li>' +
  ...
  if (!PDFViewer.IS_TOUCH_SUPPORTED) $('<li><a href="#full-screen" rel="tooltip" title="Full Screen"><i class="fa fa-arrows"/></a></li>').appendTo($navbarRight);
  ...
  $navbar.delegate('a', 'click', function(evt) {
    ...
    switch (href) {
      case '#full-screen':
        ...
        break;
      case '#zoom':
        if ($button.children('i').hasClass('fa fa-check')) return;

        $button.closest('.dropdown-menu').find('i.fa fa-check').removeClass('fa fa-check');
        $button.children('i').addClass('fa fa-check');
...
var PDFViewerPageView = function PDFViewerPageView(viewer, page) {
  ...
  var $spinner = this.$spinner = $('<i class="fa fa-spinner fa-pulse"/>').appendTo($element);
