...
import com.videoparaiso.renting.controller.PeliculaController;
import com.videoparaiso.renting.domain.Actor;
import com.videoparaiso.renting.domain.Contenido;
...
public class PeliculaData {
	...
	public void setDataSource(DataSource dataSource) {
		...
		this.simpleJdbcCall = new SimpleJdbcCall(dataSource).withCatalogName("dbo")
				.withProcedureName("VideoRent_PeliculaInsert").withoutProcedureColumnMetaDataAccess()
				.declareParameters(new SqlOutParameter("@codPelicula", Types.INTEGER))
				.declareParameters(new SqlOutParameter("@codGenero", Types.VARCHAR))
	...
	public Pelicula save(Pelicula pelicula) throws SQLException {
		SqlParameterSource parameterSource = new MapSqlParameterSource().addValue("@titulo", pelicula.getTitulo())
				.addValue("@codGenero", pelicula.getGenero().getCodGenero())
				.addValue("@totalPeliculas", pelicula.getTotalPeliculas())
				.addValue("@subtitulada", pelicula.isSubtitulada()).addValue("@estreno", pelicula.isEstreno());
	...
	public List<Pelicula> findAllMoviesByTitleAndGenre(String title, String genre) {
		String sqlSelect = "SELECT p.cod_pelicula,p.titulo,p.cod_genero,g.nombre_genero, p.total_peliculas,"
				...
				+ "  left join Actor a on pa.cod_actor = a.cod_actor" + " WHERE p.titulo like '%" + title.trim()
				+ "%' and g.nombre_genero like '%" + genre.trim() + "%'";
...
class PeliculasWithActoresExtractor implements ResultSetExtractor<List<Pelicula>> {
	...
	public void rentar(Pelicula pelicula) {
		EstadoPelicula estadoPelicula = new EstadoRentada();
		if (!pelicula.getEstado().equals(estadoPelicula)) {
			pelicula.setEstado(estadoPelicula);
			pelicula.estado();
		} else {
			throw new RuntimeException("La película está rentada.");
		}
	}

	public void devolver(Pelicula pelicula) {
		EstadoPelicula estadoPelicula = new EstadoStock();
		if (!pelicula.getEstado().equals(estadoPelicula)) {
			pelicula.setEstado(estadoPelicula);
			pelicula.estado();
		} else {
			throw new RuntimeException("La película fue devuelta.");
		}
	}
}
