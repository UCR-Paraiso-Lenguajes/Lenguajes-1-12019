...
public class PeliculaBusiness {
	...
	public Pelicula save(Pelicula pelicula) throws SQLException {
		...
		if (pelicula == null)
			throw new RuntimeException("La identificación de película es requerido");
		if (pelicula.getTitulo().length() > 200)
			throw new RuntimeException("El campo de " + "texto tiene un limite de 200 carateres");
		if (pelicula.getGenero() == null)
			throw new RuntimeException("El nombre de genero es requerido");
