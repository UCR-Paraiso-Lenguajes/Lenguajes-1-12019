...
public class InsertarPeliculaController {
	...
	public String save(@Valid PeliculaForm peliculaForm, BindingResult bindingResult, Model model) throws SQLException {
		if (bindingResult.hasErrors()) {
			...
			model.addAttribute("peliculas", peliculaBusiness.findAllMoviesByTitleAndGenere(pelicula.getTitulo(),
					"" + pelicula.getGenero().getNombreGenero()));

			// Se envia vacio para que lo busque solo con el nombre de la pelicula
			return peliculasController.findMovies(model, pelicula.getTitulo(), "");
		}
	}
