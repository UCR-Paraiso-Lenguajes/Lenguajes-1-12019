...
public class InsertarPeliculaController {
	...
	public String save(@Valid PeliculaForm peliculaForm, BindingResult bindingResult, Model model) throws SQLException 
	{
		...
		else
		{
			...
			model.addAttribute("peliculas", peliculaBusiness.findAllMoviesByTitleAndGenre(pelicula.getTitulo(), ""+pelicula.getGenero().getCodGenero()));
			
			return peliculaController.findMovies(model, pelicula.getTitulo(), peliculaBusiness.codGenero(pelicula.getGenero().getCodGenero()).getNombreGenero());
