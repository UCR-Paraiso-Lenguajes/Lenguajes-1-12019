...
public class PeliculaData {
	...
	@Transactional(readOnly=true)
	public List<Genero> obtenerNombreGenero(int cod_genero) {
		String nombre_genero="select top(1) g.cod_genero, g.nombre_genero from Genero g,Pelicula p where g.cod_genero="+cod_genero;
		return jdbcTemplate.query(nombre_genero, new GeneroWithActoresExtractor());
	}
}
...
class GeneroWithActoresExtractor implements ResultSetExtractor<List<Genero>> {
	@Override
	public List<Genero> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<Integer, Genero> map = new HashMap<Integer, Genero>();
		Genero genero = null;
		while (rs.next()) {
			Integer id = rs.getInt("cod_genero");
			genero = map.get(id);
			if (genero == null) { // new pelicula record
				genero = new Genero();
				genero.setCodGenero(id);
		...
		return new ArrayList<Genero>(map.values());
	} // extractData
