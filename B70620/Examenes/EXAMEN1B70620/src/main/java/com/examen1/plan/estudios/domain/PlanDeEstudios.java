...
public class PlanDeEstudios {
	...
	/*
	public ArrayList<Curso> obtenerRequitosDelCurso(int idCiclo, String siglasCurso) {
		if (idCiclo < 1 || idCiclo > ciclos.size())
			throw new RuntimeException("El ciclo no existe para este plan de estudios");
		Curso curso = ciclos.get(idCiclo - 1).obtenerCursoPorSigla(siglasCurso);
		
		ArrayList<Curso> listaRequisitos = new ArrayList<Curso>();
		for (int i = idCiclo - 2; i > -1; i--) {
			for (int j = 0; j < 0; j++) {
	...
	*/
	...
	public void agregarRequisitoCurso(int idCiclo, String siglaCursoAgregar, String siglaCursoRequisito) {
		if (!existeCursoEnCiclosAnteriores(idCiclo - 2, siglaCursoRequisito))
			throw new RuntimeException(
					"El plan de estudios no contiene el curso a asignar" + " como requisito en los ciclos pasados");
	...
	private boolean existeCursoEnCiclosPosteriores(int idCiclo, String siglaCursoRequisito) {
		if (idCiclo >= ciclos.size())
			return false;

		if (ciclos.get(idCiclo).existeCursoEnCiclo(siglaCursoRequisito))
			return true;
