...
public class MessageControllerRest {
	...
	@RequestMapping(value="/api/groupInfo", method=RequestMethod.GET)
    public @ResponseBody ArrayList<User> getUsersGroup(Model model, @RequestParam("idGroup") int idGroup) {
		Iterator<User> users = userBusiness.findUsersCertainRoom(idGroup);
		ArrayList<User> usersA = new ArrayList<>();
		users.forEachRemaining(usersA::add);
        return usersA;
    }
