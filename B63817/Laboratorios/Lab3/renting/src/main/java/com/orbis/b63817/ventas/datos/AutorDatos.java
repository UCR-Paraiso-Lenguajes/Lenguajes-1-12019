...
public class AutorDatos {
	...
	@Transactional(readOnly = true)
	public ArrayList<Autor> mostrarAutorPorId(int id_autor) {
		String sqlSelect = "Select id_autor,nombre_Autor, apellidos_autor, telefono " + "From Autor " + "Where id_autor ="
				+ id_autor;
	...
	public void update(int id, Autor autor) {
		...
		String sqlSelect = "UPDATE Autor SET nombre_Autor = '"+autor.getNombre_Autor()+"',"
					+ "apellidos_autor = '"+ autor.getApellidos_autor() +"' "
							+ "where id_autor = "+ autor.getId_autor();
	...
	public void delete(int id) {
		String sqlSelect = "DELETE Libro_Autor \n" + 
				"WHERE id_autor= '" +id+"';"
				+"DELETE Autor \n" + 
				"WHERE id_autor= '"+id+"'";
				jdbcTemplate.batchUpdate(sqlSelect);
...
class AutorExtractor implements ResultSetExtractor<ArrayList<Autor>> {
	@Override
	public ArrayList<Autor> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<Integer, Autor> map = new HashMap<Integer, Autor>();
		Autor Autor = null;
		while (rs.next()) {
			Integer id = rs.getInt("id_autor");
